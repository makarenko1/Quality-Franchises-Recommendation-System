...
"""
Preprocesses the MovieLens movies, ratings, and tags datasets.
...
Outputs:
    ...
    datasets/movies/movies_ratings_clean.csv
    datasets/movies/movies_tags_clean.csv
    datasets/movies/movies_cleaning_log.txt
...
def preprocess_movies(raw_dir=RAW_DIR, out_dir=OUT_DIR):
    ...
    log(
        f"Dropped nulls/blanks: movies={before[0] - len(movies)}, "
    ...
    log(f"movies: dropped {before - len(movies)} duplicate rows", log_lines)
    ...
    log(
        f"movies: transliterated {title_changed} titles and {genre_changed} genres to ASCII",
        log_lines,
    )
    ...
    log(f"movies: dropped {before - len(movies)} rows with invalid MovieID", log_lines)
    ...
    log(
        f"movies: dropped {before - len(movies)} rows with malformed Title; "
    ...
    log(f'movies: dropped {before - len(movies)} rows with "(no genres listed)"', log_lines)
    ...
    log(f"movies: dropped {before - len(movies)} rows with unknown genre tokens", log_lines)

    before = len(movies)
    movies = movies[movies["Title"].map(str.isascii) & movies["Genres"].map(str.isascii)]
    log(
        f"movies: dropped {before - len(movies)} rows with non-ASCII characters",
        log_lines,
    )
    ...
    log(f"movies: split Genres into {max_genres} columns", log_lines)
    ...
    log(f"movies: dropped {before - len(movies)} movies with no ratings", log_lines)
